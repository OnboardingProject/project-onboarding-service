...
public class OnboardingStatusController {
	...
	public ResponseEntity<ResponsePayLoad> getPreviewStatusReport(@Valid @PathVariable("projectId") @Size(min = 1 , message = "Invalid id, please provide valid id") String projectId, 
		...
		try {
			...
			return new ResponseEntity<ResponsePayLoad>(new ResponsePayLoad(statusReportObject, ProjectOnboardingConstant.API_GET_PREVIEW_REPORT_SUCCESS, ""), HttpStatus.OK);
		} catch (ProjectOnboardingException ex) {
			logger.warn("Project/User not found, preview report failed in controller");
			return new ResponseEntity<ResponsePayLoad>(new ResponsePayLoad(null, "", ex.getErrorMessage()), HttpStatus.NOT_FOUND);
		}
